---
name: us-finance-sentiment
description: Fetch structured stock sentiment across Reddit, X.com, news, and Polymarket using the Adanos Finance API. Use when the user asks how much people are talking about a stock, how hot a ticker is, how many Polymarket bets exist, whether sentiment sources are aligned, or wants to compare sentiment across tickers. Also trigger when someone says "social sentiment on TSLA", "how hot is NVDA on X.com", "how many Reddit mentions does AAPL have", "compare sentiment on AMD vs NVDA", "Polymarket bets on Microsoft", "is Reddit aligned with X on META", "stock buzz", "bullish percentage", "what is the market talking about", or asks for cross-source sentiment. READ-ONLY. Requires Adanos API key (ADANOS_API_KEY).
license: Apache-2.0
---
